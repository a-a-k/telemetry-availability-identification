"""Versioned startup correction: wait for the shared SQL database before Java startup."""
import argparse
...
from .petclinic_runtime_v3 import compose as qualified_compose, command, db_records, digest, write
from .pmx_observed_operations import read_native
from .runner import _write_csv

ROOT = Path(__file__).resolve().parents[2]
CONFIG = ROOT/'configs/petclinic_stochastic_preflight_v2.json'
PROFILE = 'spring_petclinic_microservices'


def compose(bundle, out, config, placement='colocated'):
    path = qualified_compose(bundle, out, config, placement)
    document = json.loads(path.read_text())
    services = document['services']
    services['database']['healthcheck'] = dict(
        test=['CMD', 'mysql', '--protocol=TCP', '-h127.0.0.1', '-uroot', '-ppetclinic-study',
              'petclinic', '-e', 'SELECT COUNT(*) FROM owners; SELECT COUNT(*) FROM pets; SELECT COUNT(*) FROM visits; SELECT COUNT(*) FROM vets;'],
        interval='2s', timeout='2s', retries=90, start_period='5s')
    for service in services.values():
        if service.get('depends_on') == ['database', 'collector']:
            service['depends_on'] = dict(database=dict(condition='service_healthy'), collector=dict(condition='service_started'))
    write(path, document)
    return path
...
def run(bundle,out,placement,law):
    ...
    assert [placement, law] in selected['case_subset']
    identity=dict(profile=PROFILE,placement=placement,failure_law=law,repetition=0)
    namespace=f"petclinic-tech-v2-run{os.environ['GITHUB_RUN_ID']}-a{os.environ['GITHUB_RUN_ATTEMPT']}-{placement}-{law}"
