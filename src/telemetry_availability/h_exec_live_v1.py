...
CONFIG = Path('configs/h_exec_01_v1.json')
...
def admin_command(out, command):
    with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as stream:
        stream.settimeout(5)
        stream.connect(str((out/'admin/admin.sock').resolve()))
...
def acquire(out, frozen, phase, block, slot):
    ...
    identity = dict(hypothesis='H-EXEC-01', version='v1', phase=phase, block=block, slot=slot, arm=arm,
    ...
    try:
        ...
        namespace = f'h-exec-v1-{phase}-b{block}-slot{slot}-'+os.environ['GITHUB_RUN_ID']
