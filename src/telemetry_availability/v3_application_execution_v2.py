"""Execution binding v2: declared probe layer, Valkey peer and optional descendants."""
...
CONFIG = Path('configs/v3_application_execution_v2.json')
METHOD = 'G-EXEC-ordinary-joint-technical-v2'
...
def optional_ancestor(span, indexed, spec):
    """A source-declared ignored call also makes its nested DB work optional."""
    seen = set(); current = span
    while current is not None:
        if current['span_id'] in seen:
            raise ValueError('cyclic native parent relation')
        seen.add(current['span_id']); parent = indexed.get(current['parent_id'])
        if parent is not None and parent['service'] != current['service']:
            suffixes = spec['ignored_rpc_methods_by_pair'].get(edge_id(parent['service'], current['service']), [])
            if ignored_call(current, suffixes):
                return True
        current = parent
    return False


def declared_probe_layer(declarations):
    statuses = declarations['backend_success_check_statuses']
    if statuses == ['L4OK']:
        return 'L4'
    if statuses == ['L7OK']:
        return 'L7'
    raise BindingUnsupported('unsupported declared backend success-check contract: ' + str(statuses))
...
def request_evidence(request, spans, declarations, spec):
    ...
    for span in spans:
        span_interval(span)
        optional = optional_ancestor(span, indexed, spec)
        parent = indexed.get(span['parent_id'])
        if parent and parent['service'] != span['service']:
            pair = (parent['service'], span['service'])
            calls[pair].append(dict(span=span, parent=parent, relation='cross_service_parent', optional_for_completion=optional))
            if span['service'] == declarations['target_service'] and not optional:
        ...
        if db:
            ...
            calls[(span['service'], node)].append(dict(span=span, parent=None, relation='native_db_client', optional_for_completion=optional))
            db_records[node] = provenance
        destination = client_destination(span, spec['client_route_names'])
        if destination and explicit_status(span) is False and not optional:
...
def completion_observation(pair, evidence, spec):
    ...
    records = [r for r in records if not r.get('optional_for_completion', False) and not ignored_call(r['span'], ignored)]
...
def align_probe(started, probes, times, maximum_age_ns, layer):
    ...
    return {f'probe_{r}': probe_verdict(sample[f'replica_{r}_backend_status'],
            sample[f'replica_{r}_backend_check_status'], layer=layer)['value'] for r in ('a', 'b')}, started - times[index]
        ...
        if is_replay:
            ...
            for op, spec in operations.items():
                try:
                    ...
                    expected = settings['profiles'][profile].get('unchanged_calculation_hashes', {}).get(op)
                    if expected is not None:
                        assert reports[op]['result_sha256'] == expected, 'unaffected calculation changed: ' + op
                        reports[op]['unchanged_v1_calculation_verified'] = True
