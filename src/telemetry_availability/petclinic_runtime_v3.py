...
from .petclinic_contract_v2 import execute, finalize_write, fixture_from_source, adapt_fixture

CONFIG = Path(__file__).resolve().parents[2]/'configs/petclinic_runtime_v3.json'
...
def probe(bundle, out, config):
    ...
    try:
        ...
        for control in config['controls']:
            ...
            with urllib.request.urlopen('http://localhost:18404/stats;csv', timeout=2) as response:
                backend_rows = {r['svname'][-1]: r for r in csv.DictReader(response.read().decode().splitlines())
                                if r['svname'] in ('replica_a', 'replica_b')}
            tcp_counterexamples = []
            if control in ('one_path_a', 'one_path_b'):
                paused = 'b' if control == 'one_path_a' else 'a'
                address = states[paused]['NetworkSettings']['Networks']['petclinic-study-application']['IPAddress']
                example = dict(replica=paused, paused=states[paused]['State']['Paused'], tcp_connect=False, http_health=False)
                try:
                    with socket.create_connection((address,8080), timeout=.5): example['tcp_connect']=True
                except OSError as exc:
                    example['tcp_error']=str(exc)
                try:
                    with urllib.request.urlopen(f'http://{address}:8080/actuator/health', timeout=.5) as response:
                        example['http_health']=response.status==200
                except OSError as exc:
                    example['http_error']=str(exc)
                tcp_counterexamples.append(example)
            selected = []
            ...
            wanted = {'normal': {'a','b'}, 'one_path_a': {'a'}, 'one_path_b': {'b'}, 'no_replica': set(),
                      'live_process_communication_failure': set(), 'recovered': {'a','b'}}[control]
            checks['application_health_matches_required_paths'] = set(backend_rows)=={'a','b'} and all(
                backend_rows[replica]['status'].startswith('UP' if replica in wanted else 'DOWN') for replica in ('a','b'))
            if tcp_counterexamples:
                checks['paused_tcp_success_is_not_http_readiness'] = all(r['paused'] and r['tcp_connect'] and not r['http_health'] for r in tcp_counterexamples)
            ...
            controls.append(dict(control=control, counts=count, checks=checks, container_states=states,
                                 backend_rows=backend_rows, tcp_counterexamples=tcp_counterexamples))
