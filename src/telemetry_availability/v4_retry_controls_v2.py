"""Exact backend-line repair for the same eight prespecified controls."""
...
def inject_rules(original):
    lines=original.splitlines(keepends=True)
    positions=[i for i,line in enumerate(lines) if line.rstrip('\r\n')=='backend study_replicas']
    if len(positions)!=1:raise ValueError('unexpected qualified backend declaration')
    index=positions[0]
    if not lines[index].endswith('\n'):raise ValueError('unterminated backend declaration')
    return ''.join(lines[:index+1])+RULES+''.join(lines[index+1:])
...
def run(config,runtime_config,profile,runtime,identity,path,namespace,fixture,out):
    ...
    proxy.write_text(inject_rules(original),encoding='utf-8')
