...
METHOD='B0-v3-raw-calibration-frequency-v1'
...
def identify(requests, operations):
    ...
    for row in requests:
        ...
        assert type(row['semantic_success']) is bool and type(row['timed_out']) is bool
        assert not (row['semantic_success'] and row['timed_out'])
        operation=row['operation'];attempts[operation]+=1
        successes[operation]+=row['semantic_success'];timeouts[operation]+=row['timed_out']
    forecasts={}
