...
from .h_exec_live_v2 import settings
...
def main():
    ...
    for name in ('identity.json','acquisition-summary.json','interventions.json','boundaries.json','periods.json','instrumentation-audit.json'):
        if (args.source/name).is_file():
            write(args.output/name,read(args.source/name))
    try:
        qualified=summarize(args.source,args.output,frozen)
    except Exception as exc:
        write(args.output/'failure.json',dict(qualified=False,error=f'{type(exc).__name__}: {exc}',
            phase='compact diagnostic census',main_campaigns=0))
        raise
