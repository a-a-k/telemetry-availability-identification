...
It reuses the qualified native policies and caller-context PCM bridge unchanged.
"""
import argparse
...
from .pmx_context_comparison import save_projection, extract as qualified_extract
...
VERSION = 'v3-independent-pmx-comparison-v1'
...
def projection(native, requests, profile, declared_host):
    ...
    for operation in sorted({r['operation'] for r in requests}):
        rows = [r for r in requests if r['operation'] == operation]; tick = time.perf_counter()
...
def solver_forecasts(prepared, raw):
    ...
    for record in prepared['extractions']:
        ...
        candidates.append(dict(version=VERSION, identity=record['prospective_identity'], forecasts=forecasts,
            ...
            stages=record['stages'], read_audit=record['read_audit'], solver_evidence=evidence))
    return candidates, dict(qualified=control_ok, expected_two_pass_oracle_records=16, controls=controls)
...
def main():
    ...
    if args.mode in ('extract', 'controls'):
        ...
        key = prepare(args.source.resolve(), args.options, contract, read(args.config), args.mode=='controls')
        qualified_extract(LEGACY_CONFIG, contract, key, args.jar, out)
