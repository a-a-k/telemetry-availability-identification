"""Versioned PMX projection including observed database CLIENT endpoint operations; no inferred server spans."""
...
DB_ADAPTER_VERSION = 'observed-database-client-endpoint-v1'


def database_endpoint(span):
    attrs = span.attributes
    system = attrs.get('db.system.name', attrs.get('db.system'))
    if not system or span.server:
        return None
    if str(span.native_kind).lower() not in ('3', 'client', 'span_kind_client'):
        return None
    if system != 'mysql':
        raise AdapterError('unsupported observed database system')
    ...
    if not address or port is None or not database:
        raise AdapterError('observed database client lacks explicit peer identity')
    try:
        port = int(port)
    except (TypeError, ValueError) as exc:
        raise AdapterError('invalid observed database port') from exc
    if not 1 <= port <= 65535:
        raise AdapterError('invalid observed database port')
    return [str(system), str(address), port, str(database)]
...
def project_request(spans, request, policy, declared_host):
    ...
    if policy not in ('server_and_observed_db_client',) or not declared_host:
    ...
    selected = {key: span for key, span in by_id.items() if
                span.server or database_endpoint(span) is not None}
    ...
    for key, span in selected.items():
        ...
        endpoint = database_endpoint(span)
        resources = dict(span.resource_attributes)
        service = span.service
        if endpoint is not None:
            service = 'taid.observed-db-client.' + _digest(endpoint)[:16]
            resources = {'service.instance.id': 'observed-peer-' + _digest(endpoint)[:16],
                         'adapter.observed.database.endpoint': endpoint}
        ...
        projected.append(replace(span, parent_id=parent_id, operation=label, service=service,
                                 resource_attributes=resources))
        parent_records.append({'span_id': key, 'native_span': asdict(span),
                               'representation_role': 'observed_db_client_endpoint' if endpoint else 'native_server',
                               'observed_database_endpoint': endpoint,
...
def _project_campaign_native(grouped, requests, policy, declared_host, invalid=None):
    invalid = invalid or {}
...
def project_campaign(grouped, requests, policy='server_and_observed_db_client', declared_host='declared-physical-runner', invalid=None):
    result = _project_campaign_native(grouped, requests, policy, declared_host, invalid)
    db = [r for r in result['native_audit'] if r['representation_role']=='observed_db_client_endpoint']
    result['summary'].update(adapter_version=DB_ADAPTER_VERSION,observed_database_client_spans=len(db),
        observed_database_endpoints=sorted({tuple(r['observed_database_endpoint']) for r in db}),
        fabricated_database_server_spans=0,observed_client_errors_preserved=True)
    return result
