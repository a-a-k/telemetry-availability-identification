...
CONFIG=Path('configs/b0_technical_v3_csv_v2.json')
...
def main():
    ...
    if args.stage=='prepare':
        ...
        requests=json.loads((source/'requests.json').read_bytes())
        save(out/'outcome-encoding-census.json',{field:dict(Counter(type(r[field]).__name__+':'+repr(r[field]) for r in requests)) for field in ('semantic_success','timed_out')})
