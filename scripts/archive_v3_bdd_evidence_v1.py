"""Preserve the complete CUDD comparison remotely as original ZIPs in a draft."""
import json
import os
...
SOURCES = {34583421491: ('2ff44cd313588dbabcda149ecb3990f50694e213', 'v3-bdd-comparison-v1.yml', 'success')}

PROFILES = ('deathstarbench_social_network', 'opentelemetry_demo', 'spring_petclinic_microservices', 'synthetic')
TAG = 'evidence-v3-bdd-34583421491-v1'
OUT = Path('workflow-results/bdd-archive')


def expected_names(run_id):
    return {f'v3-bdd-{role}-{profile}-{run_id}' for role in ('compact', 'full') for profile in PROFILES}


def checked_source(run, artifacts):
    require(run['id'] in SOURCES, 'undeclared CUDD source')
    ...
    require(run['head_sha'] == head and run['run_attempt'] == 1
            ...
            'wrong, incomplete or reclassified CUDD cohort')
    names = expected_names(run['id'])
    require(len(artifacts) == len(names) and len({a['id'] for a in artifacts}) == len(names)
            and {a['name'] for a in artifacts} == names, 'CUDD artifact census differs')
...
def main():
    ...
    require(len(artifacts) == 8, 'combined original artifact census differs')
    ...
    body = dict(tag_name=TAG, target_commitish=head, draft=True, prerelease=False, make_latest='false',
                name='Unpublished complete CUDD evidence: all applications and synthetic grid',
                body='Exact original ZIPs, models, certificates and timing records from the complete CUDD comparison. '
    ...
    full = OUT / 'v3-bdd-original-artifacts.zip'
    entries = []
    ...
    manifest = dict(version='v3-bdd-durable-archive-v1',
