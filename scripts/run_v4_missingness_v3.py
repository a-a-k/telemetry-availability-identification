...
def main():
    ...
    for case in cases:
        ...
        with tempfile.TemporaryDirectory(prefix='independent-missingness-') as directory:
            ...
            output=dict(source_status=source_status,version=config['version'],identity=identity,source_run=SOURCE_RUN,source_head=SOURCE_HEAD,
                run=os.environ['GITHUB_RUN_ID'],head=os.environ['GITHUB_SHA'],sources=source,
                config_sha256=sha(Path('configs/v4_missingness_execution_v3.json')),operations=operations,
