"""Verify the performance archive from compact documents and remote metadata."""
import argparse
...
def main():
    ...
    require(run['path'] == '.github/workflows/v3-performance-durable-archive-v1.yml'
    ...
    require(len(items) == 1 and items[0]['name'] == f'v3-performance-archive-compact-{args.run}'
    ...
    require(manifest['version'] == 'v3-performance-durable-archive-v1'
            and manifest['sources'] == expected and manifest['source_artifacts'] == 13
    ...
    for name in ('scripts/archive_v3_performance_evidence_v1.py', 'scripts/archive_v3_main_evidence_v1.py',
                 'scripts/archive_h_exec_evidence_v1.py', 'scripts/retain_v3_comparison_compact_v4.py',
                 '.github/workflows/v3-performance-durable-archive-v1.yml'):
    ...
    require(len(entries) == 13 and {e['artifact_id'] for e in entries} == set(by_id), 'entry census differs')
    ...
    out = Path(f'docs/evidence/v3-performance-durable-archive-{args.run}')
    ...
    record = dict(version='v3-performance-archive-compact-retention-v1', archive_run=args.run,
                  sources=expected, archive_head=run['head_sha'], artifact_id=item['id'],
                  compact_bytes=len(raw), compact_sha256=sha256(raw).hexdigest(),
                  release_id=receipt['release_id'], draft=True, source_artifacts=13,
