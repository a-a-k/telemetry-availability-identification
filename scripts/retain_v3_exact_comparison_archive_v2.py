...
import archive_v3_exact_comparison_v2 as source
...
def main():
    ...
    require(run['path'] == '.github/workflows/v3-exact-comparison-archive-v2.yml'
    ...
    require(len(items) == 1 and items[0]['name'] == f'v3-exact-comparison-v2-archive-compact-{args.run}'
    ...
    require(len(expected) == 1 and expected[0]['conclusion'] == 'success', 'source declaration differs')
    require(manifest['version'] == 'v3-exact-comparison-durable-archive-v2'
            and manifest['sources'] == expected and manifest['source_artifacts'] == 8
    ...
    for name in ('scripts/archive_v3_exact_comparison_v2.py', 'scripts/archive_v3_main_evidence_v1.py',
                 'scripts/archive_h_exec_evidence_v1.py', 'scripts/retain_v3_comparison_compact_v4.py',
                 '.github/workflows/v3-exact-comparison-archive-v2.yml'):
    ...
    for declared in expected:
        ...
        require(original['head_sha'] == declared['head'], 'declared source head differs')
    ...
    require(len(entries) == 8 and {e['artifact_id'] for e in entries} == set(by_id), 'entry census differs')
    ...
    release = checked_draft(transport.read_api(f'releases/{receipt["release_id"]}'), f'evidence-v3-exact-comparison-{expected[0]["run_id"]}-v2', manifest['archive_head'])
    ...
    out = Path(f'docs/evidence/v3-exact-comparison-v2-durable-archive-{args.run}')
    ...
    record = dict(version='v3-exact-comparison-v2-archive-compact-retention-v1', archive_run=args.run,
                  sources=expected, archive_head=run['head_sha'], artifact_id=item['id'],
                  compact_bytes=len(raw), compact_sha256=sha256(raw).hexdigest(),
                  release_id=receipt['release_id'], draft=True, source_artifacts=8,
