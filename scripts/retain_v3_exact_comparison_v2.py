...
def validate_compact(result,config):
    ...
    expected={(i,m,x,r,p) for i in ids for m in config['methods'] for x in config['representations'] for r in range(config['technical_rounds']) for p in config['phases']}
    actual=[(r['case_id'],r['method'],r['representation'],r['round'],r['phase']) for r in result['records']]
    require(len(actual)==len(expected) and set(actual)==expected and result['complete_census'],'planned record census differs')
    require(result['verification_order']=='after_all_profile_streams','verification boundary differs')
    by_id={c['case_id']:c for c in cases}
    for row in result['records']:
        if row['status']=='qualified':
            require(row['estimates']==by_id[row['case_id']]['expected'][row['phase']],'record differs from exact reference')
            require(row['kernel_ns']==row['total_ns']-row['conversion_ns'] and row['conversion_ns']>=0,'invalid conversion time')
            require(row.get('posterior_cells_valid',True),'invalid BN posterior cells')
    ...
    resource_keys=[(r['case_id'],r['method'],r['representation'],r['round']) for r in result['resources']]
    expected_resources={(i,m,x,r) for i in ids for m in config['methods'] for x in config['representations'] for r in range(config['technical_rounds'])}
    require(len(resource_keys)==len(expected_resources) and set(resource_keys)==expected_resources,'resource census differs')
    forbidden={'model','models','observation_categories','replicas','signal_ids','category_certificates','lower_witness','upper_witness','native_spans','verification_cells'}
    def walk(obj):
...
def main():
    ...
    require(run['status']=='completed' and run['run_attempt']==1 and run['path']=='.github/workflows/v3-exact-comparison-v2.yml','wrong/incomplete source run')
    source=transport.read_api(f'contents/configs/v3_exact_comparison_v2.json?ref={run["head_sha"]}')
    ...
    out=args.out or Path(f'docs/evidence/v3-exact-comparison-v2-{args.run}')
    transport.persist(out/'.gitattributes',b'* -text whitespace=-trailing-space,-space-before-tab,cr-at-eol\n')
    receipts=[]
    for profile in config['profiles']:
        item=by_name[f'v3-exact-comparison-v2-compact-{profile}-{args.run}']
    ...
    receipt=dict(version='v3-exact-comparison-v2-compact-retention-v1',run_id=args.run,head=run['head_sha'],conclusion=run['conclusion'],
