...
def main():
    ...
    root = Path(f'docs/evidence/original-aina-audit-{args.run}')
    ...
    selected = [a for a in artifacts if a['name'] == f'original-aina-audit-compact-{args.run}']
    ...
    allowed = {'summary.json','chunk-census.json','graph-control-inventory.json',
               'original-mc-controls.json','original-member-hashes.json','input-provenance.json',
               'progress.json','failure.json','resource-usage.txt'}
