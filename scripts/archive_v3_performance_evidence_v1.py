"""Preserve all three performance cohorts remotely as original ZIPs in a draft."""
import json
import os
...
SOURCES = {
    34555933526: ('a3f63a159a98afb8c0401f86414b6395e6d99275', 'v3-pipeline-benchmark-v1.yml', 'failure'),
    34556966879: ('40afca20247697950d6a1cffbfd0362c70cae424', 'v3-model-scaling-benchmark-v1.yml', 'success'),
    34558252086: ('1e77d94cb3b4ea5f074225f232ce51cd4a0f29f5', 'v3-pipeline-benchmark-v2.yml', 'success'),
}
PROFILES = ('deathstarbench_social_network', 'opentelemetry_demo', 'spring_petclinic_microservices')
TAG = 'evidence-v3-performance-34558252086-v1'
OUT = Path('workflow-results/performance-archive')


def expected_names(run_id):
    if run_id == 34556966879:
        return {f'v3-model-scaling-compact-{run_id}'}
    return {f'v3-pipeline-benchmark-{role}-{profile}-{run_id}'
            for role in ('compact', 'full') for profile in PROFILES}


def checked_source(run, artifacts):
    require(run['id'] in SOURCES, 'undeclared performance source')
    ...
    require(run['head_sha'] == head and run['run_attempt'] == 1
            ...
            'wrong, incomplete or reclassified performance cohort')
    names = expected_names(run['id'])
    require(len(artifacts) == len(names) and len({a['id'] for a in artifacts}) == len(names)
            and {a['name'] for a in artifacts} == names, 'performance artifact census differs')
...
def main():
    ...
    require(len(artifacts) == 13, 'combined original artifact census differs')
    ...
    body = dict(tag_name=TAG, target_commitish=head, draft=True, prerelease=False, make_latest='false',
                name='Unpublished complete performance evidence: original, corrected and model scaling',
                body='Exact original ZIPs from all three cohorts. Original failed copy trials retained. '
    ...
    full = OUT / 'v3-performance-original-artifacts.zip'
    entries = []
    ...
    manifest = dict(version='v3-performance-durable-archive-v1',
