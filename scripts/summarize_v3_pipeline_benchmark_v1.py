...
def export(evidence,out):
    ...
    protocol=read(evidence/receipt['artifacts'][0]['profile']/'protocol.json')
    paired=[];stage_rows=[];sources={}
...
Run {receipt['run_id']}; execution head {receipt['head']}. All27 planned timing pairs are retained. These are computational repetitions of three previously observed calibration campaigns, not new independent observations or accuracy experiments.
...
The PMX launcher has20s per operation, included in the primary measured time. Startup-subtracted values are arithmetic decompositions of that same measurement, not measurements of a rewritten faster implementation. Dependency/compiler setup is separate. Process CPU sums use disjoint child commands; memory is the largest of sequential process/children peaks, not whole-runner or parent-orchestrator RSS.

Volumes1x/2x/4x preserve structure, masks, values, durations and the empirical state law; only unique observation IDs and multiplicities change. Original point/status/bound equality to1e-12 is required. Probe rows are shared at the unchanged timestamps. This is scalability with telemetry volume, not with deployed service count, state-space complexity or time dependence. Ratios/exponents are descriptive for these finite workloads, with no asymptotic or statistical superiority claim.

The original-main accounting table sums only separately timed commands over the full240 campaigns, including the original repeated binary compiles and application-wide analyzer batches. It is an execution-work inventory, not calendar elapsed time or a matched pipeline speedup; setup policy and hardware jobs differ from the new paired benchmark.
'''
