...
def main():
    ...
    require(run['status']=='completed' and run['conclusion']=='success' and run['run_attempt']==1
            and run['path']=='.github/workflows/v3-exact-backends-report-v1.yml','wrong report workflow')
    items=t.collect_pages(f'actions/runs/{args.run}/artifacts','artifacts')
    expected={f'v3-exact-backends-tables-{args.run}',f'v3-exact-backends-retention-{args.run}'}
    require(len(items)==2 and {a['name'] for a in items}==expected,'report artifact census differs')
    item=next(a for a in items if a['name'].startswith('v3-exact-backends-tables-'))
    ...
    allowed={n+'.csv' for n in ('measurements','process_resources','stage_summary','paired_ratios','exact_bounds','failures','census','synthetic_grid','representations')}|{'REPORT.md','provenance.json'}
    members=t.check_archive(raw,item,allowed);provenance=json.loads(members['provenance.json'])
    tables=Path(f'docs/tables/v3-exact-backends-{provenance["run_id"]}')
    ...
    out=Path(f'docs/evidence/v3-exact-backends-tables-{args.run}')
    ...
    record=dict(version='v3-exact-backends-remote-tables-retention-v1',report_run=args.run,report_head=run['head_sha'],
