...
import audit_v3_preflight_compact_v2 as admission_audit
import retain_v3_comparison_compact_v2 as retention

REPO = retention.REPO
PREFLIGHT_RUN = 34447262633
PREFLIGHT_HEAD = 'e89e7ca91dd474d8cf2b8c4ed7c1ffc052d98569'
PREFLIGHT_CI = 34447089240
MAIN_TAG = 'v3-main-771601-v2-admitted'
ADMISSION = Path('configs/v3_main_admission_v1.json')
DISPATCH = Path('docs/evidence/v3-main-dispatch-v2.json')
...
def admission_record(audit, preflight, ci, execution_lock_sha):
    ...
    return dict(version='v3-main-admission-v1', protocol_sha256=retention.DESIGN_SHA,
                ...
                execution_lock_path='configs/v3_comparison_execution_v2.json',
                execution_lock_sha256=execution_lock_sha, repository_lock_count=323,
...
def verify_locks():
    subprocess.run([sys.executable, 'scripts/verify_staged_research_lock.py',
                    'configs/v3_comparison_execution_v2.json'], check=True)
...
def checklist(number, title, summary, updates):
    source=Path('docs/iterations/048-verified-remote-continuation.md').read_text(encoding='utf-8')
    ...
    text=f'# Iteration {number}: {title.replace("-"," ")}\n\n'+summary+'\n\nUnchanged criteria reuse iteration048 evidence; no publication claim is automatically promoted.\n\n'+'\n'.join(lines)+'\n'
...
def preflight():
    ...
    subprocess.run([sys.executable, 'scripts/retain_v3_comparison_compact_v2.py', '--run', str(PREFLIGHT_RUN),
    ...
    record = admission_record(audit, source, api(f'actions/runs/{PREFLIGHT_CI}'),
                              sha('configs/v3_comparison_execution_v2.json'))
    write(ADMISSION, record)
    report = Path('docs/milestones/V3_COMPARISON_PREFLIGHT_V2_RESULT.md')
    report_text = ('# Full prospective preflight v2: verified main admission\n\n'
        ...
        'The failed v1 series remains preserved. No low-error or superiority gate is used.\n\n'
    ...
    check=checklist('049','qualified-preflight-and-main-admission',
        f'Preflight{PREFLIGHT_RUN} has passed its complete retained audit; source CI and323locks verified. Main dispatch remains conditional on fresh admitted-source CI.',
        {'C09':('ПРОВЕРЕНО','Complete v2 PMX projection, controls, probability and read chain qualifies.','Scope is development preflight; independent main remains required.'),
    ...
    head = commit_paths([root, ADMISSION, report, check], 'Retain qualified v2 preflight and admit independent main240')
    ...
    record = dict(version='v3-main-dispatch-v2', run_id=main['id'], head=head, source_ref=MAIN_TAG,
...
def collect_main(run_id):
    # Other v2 runs may complete. Only the one persisted admitted dispatch is eligible.
    ...
    subprocess.run([sys.executable, 'scripts/retain_v3_comparison_compact_v2.py', '--run', str(run_id),
    ...
    paths = [root]
    record = dict(version='v3-main-compact-collection-v1', run_id=run_id, head=dispatch['head'],
    ...
    result = Path('docs/milestones/V3_MAIN_COLLECTION_V1_RESULT.md')
