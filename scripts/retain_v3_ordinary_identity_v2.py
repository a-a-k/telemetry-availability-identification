"""Retain exact compact identity/demand results; ordinary/native artifacts stay remote."""
...
RUN = 34244994519
HEAD = 'fca9e011a3722e8d39de3b4d7dd3506858a1f385'
ROOT = Path(f'docs/evidence/v3-ordinary-identity-{RUN}')
...
FILES = {'preparation': {'source-audit.json', 'resource-usage.txt'},
         'demand': {'demand-census.json', 'consumer-read-audit.json', 'resource-usage.txt'}}
...
def main():
    # The individual-run metadata endpoint twice hit TLS handshake timeouts;
    # the canonical run listing returns the same identified run record.
    ...
    selected = {f'v3-identity-{kind}-compact-{profile}-{RUN}': (kind, profile)
                for kind in FILES for profile in PROFILES}
    expected = set(selected) | {f'v3-ordinary-identity-{p}-{RUN}' for p in PROFILES}
