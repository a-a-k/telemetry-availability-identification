"""Audit compact preflight provenance and access order; no model fitting or raw reads.
...
def pmx_binary_provenance(pmx, identity, operations, expected):
    require(pmx['read-audit.json'].get('qualified_pmx_build') == expected, 'PMX qualified repair provenance differs')
    records = pmx['costs.json']['extraction_and_bridge_records']
    wanted = {'campaign-'+digest(identity)[:20]+'--'+op for op in operations}
    require(len(records)==len(wanted) and {r['case_id'] for r in records}==wanted, 'PMX extraction cost case census differs')
    for record in records:
        execution=record.get('execution', {})
        require(record.get('qualified') is True and execution.get('exit_code')==0, 'PMX extraction did not qualify')
        require(execution.get('jar_sha256')==expected['derived_jar_sha256'], 'PMX executed a different binary')
        require(execution.get('startup_seconds')==20 and execution.get('internal_watchdog_seconds')==1800,
                'PMX production launcher settings differ')


def audit(root, expected_run, expected_head):
    manifest = read(root/'verified-archives.json')
    require(manifest['run_id'] == expected_run and manifest['head'] == expected_head and manifest['mode'] == 'preflight', 'evidence identity differs')
    require(manifest['protocol_sha256'] == retention.DESIGN_SHA and manifest['planned_campaigns'] == 6, 'preflight design differs')
    choices, cases = retention.selections(expected_run, 'preflight')
    ...
    require(result['head'] == expected_head and result['run_id'] == str(expected_run) and result['mode'] == 'preflight' and result['data_role'] == 'development_preflight', 'analysis provenance differs')
    require(result['main_campaigns'] == 0 and result['campaign_count'] == 6 and result['operation_cells_per_method'] == 20, 'analysis planned census differs')
    require(result['admission_candidate'] is True and result['candidate_evaluator_integrity'] is True and result['nonempty_primary_family'] is True and result['qualification_uses_forecast_error'] is False, 'remote preflight gate not qualified')
    ...
    require(expected_build['qualified'] is True, 'PMX correction is not qualified')
    proof_path=Path(expected_build['qualification_evidence'])
    proof=read(proof_path)
    require(sha(proof_path)==expected_build['qualification_audit_sha256'] and proof['qualified'] is True
            and proof['derived_jar_sha256']==expected_build['derived_jar_sha256']
            and proof['run_id']==str(expected_build['qualification_run']), 'PMX correction qualification proof differs')
    oracle_count=0
    ...
    for index, profile in enumerate(profiles,1):
        ...
        require(controls.get('qualified_pmx_build')==expected_build, 'PMX control binary provenance differs')
    ...
    for case in cases:
        ...
        require(pmx['candidates.json']['version']=='v3-independent-pmx-comparison-v3', 'PMX bridge version differs')
        ...
        pmx_binary_provenance(pmx,identity,ops,expected_build)
    ...
    require(len(freeze_jobs)==6 and len(evaluation_jobs)==6 and all(j['conclusion']=='success' for j in freeze_jobs+evaluation_jobs), 'freeze/evaluator job census differs')
    ...
    return dict(version='v3-compact-preflight-admission-audit-v3',qualified=True,run_id=expected_run,head=expected_head,protocol_sha256=retention.DESIGN_SHA,
        evidence_manifest_sha256=sha(root/'verified-archives.json'),comparison_sha256=sha(result_path),campaigns=6,operation_cells_per_method=20,
        oracle_solver_records=oracle_count,current_method_slots=200,qualification_uses_forecast_error=False,
        main_admission_created=False,main_campaigns=0,fit_or_replay_executed_locally=False,
...
def main():
    ...
    retention.persist(args.evidence/'admission-audit.json',retention.encoded(report))
