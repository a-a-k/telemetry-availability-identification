...
RUN = 34215477704
HEAD = 'e257c8e64e6fab7ca57c93ea20599b55e97e2102'
ROOT = Path(f'docs/evidence/petclinic-graph-observation-{RUN}')
...
def main():
    ...
    selected = {f'petclinic-graph-{role}-{placement}-{RUN}': f'{role}-{placement}'
                for role in ('fit', 'source', 'replay') for placement in ('colocated', 'split')}
    selected[f'petclinic-graph-qualification-{RUN}'] = 'qualification'
