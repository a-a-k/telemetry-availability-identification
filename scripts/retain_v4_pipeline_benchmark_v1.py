...
def main():
    ...
    require(run['path'] in ('.github/workflows/v4-pipeline-benchmark-v1.yml',)
            and run['run_attempt']==1 and run['status']=='completed', 'wrong or unfinished benchmark')
    protocol_raw=subprocess.check_output(['git','show',run['head_sha']+':configs/v4_pipeline_benchmark_v1.json'])
    ...
    root=Path(f'docs/evidence/v4-pipeline-benchmark-{args.run}')
    ...
    for profile in protocol['applications']:
        key=f'v4-pipeline-benchmark-compact-{profile}-{args.run}'
        ...
        for row in result['records']:
            ...
            if row['status']=='qualified':
                ...
                require(row['extra_verification_after_all_measurements'] is True and set(row['graph']['stages'])=={'build'}
                    and set(row['pmx']['stages'])=={'extract','collect','solve','save'},'extra verification entered the primary timer')
                require(set(row['extra_verification_stages'])=={'graph_replay','pmx_repeat'}
                    and all(s['exit_code']==0 and not s['timed_out']for s in row['extra_verification_stages'].values()),'missing independent checks')
    ...
    record=dict(version='v4-pipeline-benchmark-compact-retention-v1',run_id=args.run,head=run['head_sha'],
