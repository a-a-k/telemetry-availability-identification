...
RUN = 34249604323
HEAD = 'a7102d01fa4a3b6c2f0f438f6730c1656b1bba35'
ROOT = Path(f'docs/evidence/v3-application-execution-{RUN}')
...
def main():
    ...
    selected = {f'v3-execution-{kind}-compact-{profile}-{RUN}': (kind, profile)
                for kind in FILES for profile in PROFILES}
    expected = set(selected) | {f'v3-execution-{kind}-{p}-{RUN}' for kind in ('ordinary', 'model') for p in PROFILES}
