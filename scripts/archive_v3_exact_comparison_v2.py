...
def expected_names(run_id):
    return {f'v3-exact-comparison-v2-{role}-{profile}-{run_id}' for role in ('compact', 'full') for profile in PROFILES}


def checked_source(run, artifacts):
    head, workflow, conclusion = run['head_sha'], 'v3-exact-comparison-v2.yml', 'success'
...
def main():
    ...
    original = transport.read_api(f'actions/runs/{source_run}')
    SOURCES = {source_run: (original['head_sha'], 'v3-exact-comparison-v2.yml', 'success')}
    TAG = f'evidence-v3-exact-comparison-{source_run}-v2'
    artifacts = []
    ...
    body = dict(tag_name=TAG, target_commitish=head, draft=True, prerelease=False, make_latest='false',
                ...
                body='All original ZIPs of the corrected fixed comparison, with full timed worker outputs and compact verified results. '
    ...
    full = OUT / 'v3-exact-comparison-v2-original-artifacts.zip'
    entries = []
    ...
    manifest = dict(version='v3-exact-comparison-durable-archive-v2',
