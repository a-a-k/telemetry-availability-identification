...
DESIGN = Path('configs/v3_comparison_design_v3.json')
DESIGN_SHA = '6b60a07faad6778284f36bebc86a3f8a201c4b19319f006206b1e63b5ab8b548'
MAX_ARCHIVE = 100_000_000
MAX_MEMBER = 150_000_000
MAX_EXPANDED = 200_000_000


# Only retrieval pacing differs from v3. Exact selection, byte verification and
# retained evidence format remain the frozen v3 implementation.
RATE_RESERVE = 32
_rate_budget = 0
...
def core_budget():
    value = json.loads(subprocess.check_output(['gh', 'api', 'rate_limit']))['resources']['core']
    if (type(value.get('remaining')) is not int or value['remaining'] < 0 or
            type(value.get('reset')) is not int or value['reset'] <= 0):
        raise ValueError('invalid provider core-rate budget')
    return value


def wait_for_reset(reset):
    # This coordinator runs remotely. Short chunks also keep its progress visible.
    until = max(time.time() + 1.0, reset + 2.0)
    while time.time() < until:
        remaining = until - time.time()
        print(json.dumps(dict(stage='compact_retention_rate_wait',
                              seconds_until_reset=round(remaining, 1))), flush=True)
        time.sleep(min(50.0, remaining))


def api(path):
    global _rate_budget, _rate_checked_at
    if _rate_budget <= 0 or time.monotonic() - _rate_checked_at >= 15:
        budget = core_budget()
        while budget['remaining'] <= RATE_RESERVE:
            wait_for_reset(budget['reset'])
            budget = core_budget()
        _rate_budget = min(16, budget['remaining'] - RATE_RESERVE)
        _rate_checked_at = time.monotonic()
    _rate_budget -= 1
    # No write endpoint or uncertain-request retry is introduced. The unchanged
    # caller decides which exact terminal-run metadata or allowlisted ZIP to read.
