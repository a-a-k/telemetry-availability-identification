...
RUN = 34230603338
HEAD = '3477c79c5ca0037979a146346c6b49b6215bac57'
ROOT = Path(f'docs/evidence/h-exec-v2-study-{RUN}')
...
def main():
    ...
    assert run['head_sha'] == HEAD and run['status'] == 'completed' and run['conclusion'] in ('success','failure','cancelled')
    ...
    selected = {f'h-exec-compact-study-b{block}-s{slot}-{RUN}': f'block-{block}-slot-{slot}' for block in range(8) for slot in range(4)}
    selected[f'h-exec-study-{RUN}']='analysis'
    selected[f'h-exec-plan-study-{RUN}']='plan'
    allowed = {'census.json','summary.json','static-controls.json','identity.json','boundaries.json',
        ...
        'qualification.json','study-analysis.json','plan.json','failure.json'}
    records = []
    ...
    for item in artifacts:
        ...
        with zipfile.ZipFile(io.BytesIO(data)) as archive:
            ...
            for member in members:
                ...
                if member.filename not in {'census.json','static-controls.json','docker-stats.jsonl','compose-ps.json'}:
    ...
    save(ROOT/'missing-compact-artifacts.json', sorted(set(selected)-{r['name'] for r in records}))
    ...
    print(json.dumps(dict(run=RUN,conclusion=run["conclusion"],compact_archives=len(records),total_bytes=sum(r["bytes"] for r in records))))
