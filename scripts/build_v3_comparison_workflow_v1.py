...
def setup():
    return [dict(uses='actions/checkout@v7', **{'with': {'persist-credentials': False}}),
        ...
        run('Verify complete frozen comparison implementation', 'python scripts/verify_staged_research_lock.py configs/v3_comparison_execution_v1.json')]
...
workflow = dict(name='V3 prospective comparison v1', on={'workflow_dispatch': {'inputs': {
...
jobs['plan'] = {'runs-on': 'ubuntu-latest', 'timeout-minutes': 10,
    ...
    'steps': setup()+[run('Validate planned identities and main admission',
        f'python -m telemetry_availability.v3_comparison_orchestration_v1 matrix --mode {MODE}', id='matrix')]}
...
/usr/bin/time -v -o "$out/start-resource-usage.txt" docker compose -f "$out/pinned-compose.json" up -d --no-build --wait --wait-timeout 360''',
    ...
    run('Acquire all attempts and seal physically separate observation roles',
        ...
        'python -m telemetry_availability.v3_comparison_acquisition_v1 --profile "${{ matrix.profile }}" '
...
graph = setup()+[download('v3-comparison-ordinary-'+KEY, 'workflow-input/ordinary'),
    run('Build all graph variants and B0 without evaluator inputs',
        ...
        '--output workflow-results/graph --config configs/v3_comparison_execution_v1.json'),
    run('Reproduce all saved-model probabilities in a fresh restricted process',
        ...
        '--output workflow-results/graph/compact --config configs/v3_comparison_execution_v1.json'),
...
pmx_command = ('python -m telemetry_availability.v3_comparison_pmx_v1 {mode} --source workflow-input/pmx-native '
    '--out workflow-results/extraction --config configs/v3_comparison_execution_v1.json '
    ...
    if step.get('name') == 'Download the retained artificial PCM contract':
        steps.extend([download('v3-comparison-pmx-extraction-'+PROFILE+'--*', 'workflow-input/extractions', True),
            ...
            run('Census independently extracted models for this application',
                f'python -m telemetry_availability.v3_comparison_pmx_v1 collect --source workflow-input/extractions --profile "{PROFILE}" --out workflow-input/petclinic-pmx-contract')])
...
steps.extend([run('Freeze every independent PMX forecast or absence after known-probability controls',
    'python -m telemetry_availability.v3_comparison_pmx_v1 freeze --source workflow-input/petclinic-pmx-contract '
...
freeze = setup()+[download('v3-comparison-receipt-'+KEY, 'workflow-input/receipt'),
    ...
    run('Seal all ten-method slots before any evaluator download',
        f'python -m telemetry_availability.v3_comparison_orchestration_v1 freeze --mode {MODE} --key "{KEY}" --source workflow-input --out workflow-results/frozen'),
...
evaluation = setup()+[download('v3-comparison-frozen-'+KEY, 'workflow-input/frozen'),
    ...
    run('Open exact sealed evaluator and score immutable forecasts',
        f'python -m telemetry_availability.v3_comparison_orchestration_v1 evaluate --mode {MODE} --key "{KEY}" '
...
jobs['analysis'] = {'needs': ['plan', 'evaluate'], 'if': "${{ always() && needs.plan.result == 'success' }}",
    'runs-on': 'ubuntu-latest', 'timeout-minutes': 30, 'steps': setup()+[
        ...
        run('Reconcile every planned identity, primary/stable/transfer coverage and costs',
            f'python -m telemetry_availability.v3_comparison_orchestration_v1 summarize --mode {MODE} --source workflow-input --out workflow-results/comparison'),
...
Path('.github/workflows/v3-prospective-comparison-v1.yml').write_bytes(
