...
the old binding through earlier proxy verdicts and rescued transport failures.
...
The result before the symbolic comparison is pinned by annotated tag **v1** (`0c83a82`).
The [CUDD comparison](docs/milestones/V3_BDD_COMPARISON_RESULT.md) now covers all
761 saved application models and 27 artificial grid positions, with identical joint
observations, exact bounds, contrasts and validated witnesses. Our first solve is
faster on the application models including BDD construction; ready-diagram queries
favour CUDD, as do first solves at 8–10 unknown controls in the artificial grid.
All three fixed policies, CPU, memory and every grid row are reported. Completion
reduction alone does not establish superiority over symbolic algorithms.
