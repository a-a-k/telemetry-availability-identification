...
| F: explicit graph, replica, state-law, operation and observation semantics | Makes the scope of the earlier graph estimators and execution refinement explicit; a renamed tuple is not the contribution | [Execution specification and proofs](../GRAPH_EXECUTION_MODEL_V1.md), source-bound adapters and [methods chapter](../ARTICLE_METHODS_V3.md) | The origin is the [Krasnovsky–Maslovskaya preprint](https://arxiv.org/abs/2607.00740v1), without an accepted-MODELS-publication claim. Do not imply a universal reliability formalism is new |
