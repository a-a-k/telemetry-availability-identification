...
Draft for review. The independent main comparison has not yet supplied its audited result section.
...
| F: explicit graph, replica, state-law, operation and observation semantics | Makes the scope of the earlier graph estimators and execution refinement explicit; a renamed tuple is not the contribution | [Execution specification and proofs](../GRAPH_EXECUTION_MODEL_V1.md), source-bound adapters and [methods chapter](../ARTICLE_METHODS_V3.md) | Attribute the MODELS manuscript/preprint as an origin, without an accepted-publication claim. Do not imply a universal reliability formalism is new |
