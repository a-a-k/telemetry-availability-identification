...
Full native/raw archives were interpreted only in GitHub Actions and remain remote. The audit process used 132.41 seconds wall time, 48.03 seconds user CPU, 2.73 seconds system CPU and peak RSS 199,464 KiB. These are audit costs, not prospective model or monitoring-overhead measurements. Remote Actions artifact retention is 90 days. The subsequent [durable archive completed on 10 September 2026](H_EXEC_DURABLE_ARCHIVE_V1_RESULT.md) preserves all 68 original study/audit ZIPs unchanged in an unpublished repository draft; it adds no independent study data and is not a public deposit.
