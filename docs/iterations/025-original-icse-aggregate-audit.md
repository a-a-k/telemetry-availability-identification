# Итерация025 — original ICSE scalar/table audit

2026-09-08; v3 authority. [Evidence](../milestones/ORIGINAL_ICSE_AGGREGATE_AUDIT.md). Reproduced all250 source CSV rows from750 compact scalar JSON and all40 printed table cells. Original run identity remains unestablished: surviving20410544665 uses only20 jobs and is not substituted. No original live probe denominators claimed. Main0; H-EXEC34230603338 continues.

| ID | Статус | Доказательство / область | Остаток |
| --- | --- | --- | --- |
| C01 | ЧАСТИЧНО | No-ML control chain и exact I2, регрессия не primary. | Прикладной G-ID/G*. |
| C02 | ЧАСТИЧНО | Технические G/R/P/Φ и joint observation law пройдены для3 apps; source-declared PostgreSQL peer остался обязательным узлом. | Обоснование business adequacy и окончательный основной класс. |
| C03 | ЧАСТИЧНО | AINA full raw audit retained; ICSE250 CSV jobs/750 scalar files reconcile exactly,40 published table cells reproduced at printed precision. | Original ICSE250-job run/ZIP identity and raw denominators unavailable; retained20-job rerun explicitly excluded as source. |
| C04 | ЧАСТИЧНО | Ten-contract evidence сохранено; новый graph build/replay есть для create_visit, read_user_timeline, browse_product. | Поддержка всех10 событий, включая multi-call checkout. |
| C05 | ЧАСТИЧНО | Все 160 M7 campaigns, 576000 all-sequence attempts и 34560 census slots/угол сохранены; новые stable windows 435761 attempts. | Полная prospective v3 main матрица. |
| C06 | ЧАСТИЧНО | Petclinic runtime/8 conditions/PMX сохраняются; DB role явно ограничена. | Прикладной G*/transfer/cost. |
| C07 | ЧАСТИЧНО | 168 paired refits используют прежнюю physical interpretation; source prefix defect и unaffected trace_only зависимость установлены. | Новый ordinary G* не должен наследовать privileged truth; causal adequacy отдельно. |
| C08 | ЧАСТИЧНО | Обе новые graph chains прошли model-only replay;6/1 reads; по2 required-node mutation и equivalence controls. | Independent adequacy при отказах, неопределённость будущих вероятностей. |
| C09 | ЧАСТИЧНО | M9X correction пересчитана: все108 candidates/триviews, 24 PMX objects неизменны, empty OTel conditional common0/6 сохранён. | Независимое main G* comparison; corrected M7 не новый G*. |
| C10 | ЧАСТИЧНО | G из native parent edges; OTel DB CLIENT owner/system/address наблюдаемы, namespace/port дополнены совместимыми явными declarations. | Полный same-business-event estimator, background states и G0 adaptation. |
| C11 | ЧАСТИЧНО | V3 draft 240/800, шесть contrasts, 99.1666667%. | Frozen executable main protocol, запуск после admission. |
| C12 | ЧАСТИЧНО | Новые2 builders читают только6 ordinary data files,2 replayers только model.json; blocked/evaluator/legacy0, seals/forecasts совпали. | Полная future candidate→evaluator main цепочка. |
| C13 | ЧАСТИЧНО | Все17856 forecast slots сохранены; full M7 rescore four corners/34560 slots каждый; нет исключения 30 raw-point mismatches из истории. | Main attempted census и все новые absences. |
| C14 | ЧАСТИЧНО | Original sampled_mixed/stable Brier contrast пересчитан: Δ.000212314, CI[-.001702533,.002127161], p.715273, 117 pairs. | Новая six-contrast main uncertainty; historical nonsignificance не equivalence. |
| C15 | ЧАСТИЧНО | Proposed/full current coverage370/480 неизменна; own/common/complete-campaign и empty PMX contrasts явно разделены. | Main own/common support. |
| C16 | ЧАСТИЧНО | Transfer новой empirical-joint версии явно unsupported без дополнительных law assumptions, target inputs не используются. | Обоснованные transfer estimates/bounds и change error. |
| C17 | ЧАСТИЧНО | Сохранены source/build/replay wall/user/system/RSS; inner calculation time отделён от cold process/import costs. | Comparable end-to-end main costs; off/on и size curve или исключение claims. |
| C18 | ЧАСТИЧНО | Correction закрыта для160 M7/8 Petclinic/M9X: affected10080 strict parity,7776 exact historical carry-forward, full rescore; old failures сохранены. | Auxiliary v0 consumers имеют static inventory и требуют reanalysis перед reuse; future corrections. |
| C19 | ЧАСТИЧНО | AINA exact compact1511261bytes,API/member hashes; original payload remote; prior403 failure compact739bytes preserved. | Final archive и долговременное хранение. |
| C20 | ПРОВЕРЕНО | По-прежнему три приложения, ≤2 deep mechanisms, main=0; только bounded controls/read-only audit. | Соблюдать лимит в дальнейшем. |
| C21 | ЧАСТИЧНО | Уточнены source semantics, privileged-role limits и population/causal distinction. | Final claim audit. |
| C22 | ЧАСТИЧНО | Все30 criteria обновлены011–025; ICSE scalar reproduction отделена от original runtime/raw reproduction. | Завершение исследования/пакета. |
| F01 | ЧАСТИЧНО | Parser correction изменила OTel checkout MAE24.46223→24.48635pp; она не объясняет крупное исходное отклонение. | Проверить H-EXEC и альтернативы посредством intervention. |
| F02 | ЧАСТИЧНО | Контроль actual likelihood: одинаковое source r=.8, разные transfer .832/.808; semantic/identification bounds не названы новизной. | Полный основной класс и novelty audit. |
| F03 | ПРОВЕРЕНО | Technical scopes: Petclinic create_visit2 placements, DeathStar read_user_timeline и OTel browse_product; native graph→saved model→fresh replay, structural/equivalence passed. | Основной G* и остальные операции; техническая проверка не adequacy. |
| F04 | ЧАСТИЧНО | H-EXEC v2 technical run34228790846 success4/4, все census/native/routes/negative/intervention checks passed. Study34230603338 начат с тем же protocol hash. | Полные32 prospective study campaigns и frozen analysis; technical block исключён из inference. |
| F05 | ЧАСТИЧНО | Forecast/window изменения разложены по2×2; исходные outcomecounts не изменены; baseline arithmetic exact. | Business adequacy нового graph method и sampling/identification error отдельно. |
| F06 | НЕ ПРОВЕРЕНО | Нет необоснованного назначения refinement или superiority. | ≤2 подтверждённых механизма, versions/ablations/G*. |
| F07 | ЧАСТИЧНО | AINA source law/predicates fully enumerated: async Phi invariant for all allowed states;2500 MC errors within family diagnostic. Camera-ready0.001pp literal bound not reproduced. | AINA runtime/timing and ICSE original250-job run/raw logs remain; latest20-job rerun is not substituted. |
| F08 | ЧАСТИЧНО | Технические графовые кандидаты теперь есть для3/3 apps с explicit assumptions; G* ещё не выбран. | Полные semantic/identification/causal/variant main admission требования. |

Next: finish32 H-EXEC study cells and prospective analysis; remote AINA timing feasibility diagnostic is frozen separately before reading raw error strings.
