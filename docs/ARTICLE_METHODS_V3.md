...
Editorial draft prepared on 10 September 2026. This chapter follows the [author's v3 correction](SIMPAT_CORRECTION_V3.md) and the frozen [comparison bindings](V3_FINAL_COMPARISON_BINDINGS_V1.md). It describes the implemented method, not a completed predictive-validation result. The current six-campaign full-duration preflight is run `34460574221` at source commit `27db98d32b683b14969f2734f19cf344c95ae1a7`; that preflight is now qualified and independent main run `34465226083` is executing from admitted source `d857ea65ca9da7fa9ae4ee1198317487475246a7`. The earlier two preflights and the qualified PMX repair remain documented development evidence. Subsequent result documents must supply the actual run, frozen source, attempted census and evidence manifest. The earlier [methods draft](ARTICLE_METHODS_DRAFT.md) remains a record of the independent-primitive/log-linear development and does not define the final Gstar estimator.

The method connects a formal stochastic execution model to automatic construction and identification from telemetry, then to a point probability or attainable bounds. It builds the operation graph from native observations under declared source semantics, binds replicas, retains a joint masked category for every calibration attempt and computes the requested functional without machine learning or independent-coordinate factorization. The saved model supports separate calculation and replay. Structural binding failures remain explicit; attainable probability bounds concern models whose structural and observation contracts are defined. The following sections specify this chain, its identification and enumeration guarantees, and the independent evaluation of adequacy, accuracy, support and costs.
