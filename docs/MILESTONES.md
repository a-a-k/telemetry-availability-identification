...
| M9I | Audit embedded PMX failure-detection semantics against the retained M9H input/log boundary | Protocol frozen; remote audit pending | `M9I_PMX_FAILURE_SEMANTICS_PROTOCOL.md`; complete source census in four byte-pinned bundles plus exact retained raw/stdout/PCM boundary, no dynamic PMX invocation, exactly three 360-minute jobs |
