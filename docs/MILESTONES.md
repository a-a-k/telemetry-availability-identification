...
| M9H | Test the exact source-declared PMX `main:main` entrypoint and frozen failure mechanism | Protocol frozen; remote run pending | `M9H_PMX_SOURCE_ENTRYPOINT_PROTOCOL.md`; one source-derived command, fixed startup stabilization, valid Gogo exit, conditional confirmations, exactly three 360-minute jobs |
