...
| M9M | Test source- and learner-grounded request-level checkout routing models behind a frozen learner/evaluator boundary | Preregistered; remote run pending | Three-call source audit, outcome-blind baseline trace footprints, candidate-specific learner refits, and an immutable prediction artifact before retained test access; exploratory single-operation evidence only |
