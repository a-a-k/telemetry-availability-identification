...
| M9F | Audit reproducibility and semantic fit of the PMX performability route before choosing a manual continuation | Complete | `milestones/M9F_PMX_PERFORMABILITY_AUDIT.md`; accepted run 34041926658 found four repeat-consistent 900-second launcher timeouts, no generated files, and a recoverable source snapshot but not a standalone later build or exact historical container chain. This is a scoped application/reproducibility cost, not a verdict on PMX or Palladio |
| M9G | Recover the pinned PMX launcher route and measure the application adapter/information delta on a separate track | Complete | `milestones/M9G_PMX_RECOVERY_AND_APPLICATION_DELTA.md`; historical output recovered, four guessed commands rejected, source-declared `main:main` isolated for M9H, 0/160 raw learner streams, and four learner-only raw schemas adaptable but without direct PMX instrumentation semantics |
| M9H | Test the exact source-declared PMX `main:main` entrypoint and frozen failure mechanism | Complete | `milestones/M9H_PMX_SOURCE_ENTRYPOINT.md`; first-attempt run 34050900275 reproduced the historical five-model signature three times and exited cleanly, while both one-error controls remained identical to the original, localizing the open question to failure semantics rather than launcher recovery |
...
| M9L | Discriminate observable causes within checkout's route-up residual | Complete | `milestones/M9L_CHECKOUT_FAILURE_CAUSES.md`; first-attempt run 34085244404 localized `+0.171370` of the calibration residual to observed replica-path loss with complete trace resolution, while preserving the per-call timing limitation |
| M9M | Test source- and learner-grounded request-level checkout routing models behind a frozen learner/evaluator boundary | Complete | `milestones/M9M_CHECKOUT_ROUTING_MODEL.md`; accepted run 34089829138 bracketed observations between static AND and OR in 39/40 cells, but the primary trace model underpredicted by `-0.068427` and selected the temporal-failover branch |
| M9N | Test learner-only temporal checkout forecasts and a separately labelled conditional mechanism diagnostic | Complete | `milestones/M9N_CHECKOUT_TEMPORAL_FAILOVER.md`; first-attempt run 34111689492 closed the primary signed gap and passed the frozen B0 comparison, while near-identical state-only marginal forecasts require matched controls in independent confirmation |
| M9O | Preregister matched independent temporal confirmation and size the prospective campaign matrix | Complete as design; confirmation reported in M9P | `milestones/M9O_TEMPORAL_CONFIRMATION_DESIGN.md`; first-attempt run 34113304974 selected and audited 30 repetitions per stratum, 120 fresh campaigns, with 62 nominal runner-hours and a separate no-fit preflight gate |
| M9P | Independent temporal confirmation with separate no-fit preflight | Complete | [Main report](milestones/M9P_INDEPENDENT_TEMPORAL_CONFIRMATION.md); accepted preflight 34115470738; run 34118845320 retains all 120 cells and passes all five gates: conditional effect replicated, marginal temporal/state practical equivalence, mean signed error adequate within ±0.03; all 485 source/summary artifacts audited |
| M9Q | Qualify an explicit observed-operation adapter and audit four historical learner samples | Complete as census; full application projection unresolved | `milestones/M9Q_PMX_OBSERVED_OPERATION_CONFORMANCE.md` and `milestones/M9Q_PMX_APPLICATION_CENSUS.md`; original conformance failure and explicit repair retained; run 34125799582 has two no-SERVER-input cases and two exact observed-operation inventories with workload mismatches; retained audit 34128536931 changes no gate and invokes no extractor |
| M9R | Audit native PMX failure-type serialization and inclusive-error composition through the pinned Palladio solver | Complete as bounded census; solver correspondence unresolved | `milestones/M9R_PMX_INCLUSIVE_ERROR_COMPOSITION.md`; run 34131147623 retains all 16 attempts: four missing-type failures and twelve returned zero-mass results with loop-PMF errors; zero valid reliability answers and zero new PMX invocations |
