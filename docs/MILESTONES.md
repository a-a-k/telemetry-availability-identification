...
| M9J | Test the exact source-implied surviving-carrier failure contract | Protocol frozen; remote run pending | Matched carrier `error="false"`/`error="true"` controls plus the retained child-error witness; independent 0 versus 0.1 mechanism oracles, no application accuracy or new collection; final bounded PMX diagnostic before M9K |
| M9K | Localize the retained proposed-model overprediction on one operation | Planned after M9J | Checkout fixed from M8B before re-opening evidence; decompose the frozen prediction without tuning M7, then combine this result with M9J to select any further experiment |
