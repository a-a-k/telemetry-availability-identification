"""Artificial compact-only controls for prospective admission evidence auditing."""
...
import audit_v3_preflight_compact_v1 as audit
...
def fixture(root, defect=None):
    choices,cases=retention.selections(RUN,'preflight')
    ...
    for case in cases:
        ...
        if defect=='read':graph_audit['actual_data_reads'][0]='/artificial/workflow-input/evaluator/native.json'
        ...
        put(base+'/graph-compact','replay.json',dict(candidate_seal_sha256=graph_hash,result=dict(exact=defect!='replay'),
            read_audit=reads('workflow-results/graph/candidates',('candidates.json','models.json','costs.json','read-audit.json','seal.json'))))
        pmx_candidate=dict(version='v3-independent-pmx-comparison-v1',identity=identity,forecasts={op:{m:dict(status='ok',probability=1.) for m in ('PMX','PMX_inclusive')} for op in ops},
    ...
    result=dict(head=HEAD,run_id=str(RUN),mode='preflight',data_role='development_preflight',main_campaigns=0,campaign_count=6,operation_cells_per_method=20,
    ...
    for index in range(6):
        ...
        jobs.append(dict(id=index+6,name=f'evaluate (artificial-{index})',conclusion='success',
    ...
    manifest=dict(run_id=RUN,head=HEAD,mode='preflight',protocol_sha256=retention.DESIGN_SHA,planned_campaigns=6,
...
class CompactAdmissionAuditTests(unittest.TestCase):
    def test_complete_artificial_compact_chain(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result['current_method_slots'],200)
    ...
    def test_authenticated_remote_reports_cannot_hide_role_replay_oracle_or_order_fault(self):
        for defect,reason in [('read','actual data read path differs'),('replay','graph replay not exact'),('oracle','known probability not reproduced'),('order','evaluator started before all freezes completed')]:
