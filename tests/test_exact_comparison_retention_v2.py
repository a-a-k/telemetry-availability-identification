...
class RetentionTests(unittest.TestCase):
    def setUp(self):
        ...
        self.config=dict(profiles=['control'],expected_models=[1],expected_original_absences=[0],
                         methods=['control'],representations=['original'],technical_rounds=1,phases=['initial'])
        self.result=dict(profile='control',verification_order='after_all_profile_streams',cases=[dict(case_id='tiny',expected=dict(initial=ex))],absent=[],
                         records=[dict(case_id='tiny',method='control',representation='original',round=0,phase='initial',status='qualified',
                                       estimates=deepcopy(ex),total_ns=2,kernel_ns=1,conversion_ns=1,query_including_update_ns=1,key_check_ns=0)],
                         resources=[dict(case_id='tiny',method='control',representation='original',round=0)],complete_census=True,qualified=True)
    ...
    def test_rejects_wrong_representation(self):
        self.result['records'][0]['representation']='foreign'
        with self.assertRaisesRegex(ValueError,'census'):validate_compact(self.result,self.config)

    def test_rejects_wrong_verification_boundary(self):
        self.result['verification_order']='before_query'
        with self.assertRaisesRegex(ValueError,'verification boundary'):validate_compact(self.result,self.config)

    def test_rejects_conversion_subtraction(self):
        self.result['records'][0]['kernel_ns']=2
        with self.assertRaisesRegex(ValueError,'conversion'):validate_compact(self.result,self.config)
